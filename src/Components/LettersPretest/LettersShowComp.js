...
const LetterShowComp=()=>{
    ...
    const [FinalResultPopupDisplay,SetFinalResultPopupDisplay] = useState({visibility:"hidden",alignContent:"center"});
    ...
    function PassAudioText(data){
        ...
        if (String(lettersShow) === String(data).toUpperCase()) {
            ...
            console.log("Result is",countRef.current);
            SetLettersSpeeled(data);
            return countRef.current++;
        };
        SetLettersSpeeled(data);
    ...
    const NextClicked=()=>{
        if (isStart) {
            ...
            if (Indexcounter.current<=Letters.length) {
                ...
                //RestRef.current = "InputTriggered";
                console.log(Ltters);
    ...
    async function LetterPretestScoreSubmit(){
        ...
        const SendData = await fetch('https://iamadityachakraborty.pythonanywhere.com/LetterTestScore',{
            method:"POST",
    ...
    useEffect(()=>{
        Say("You will be showned 26 letters back to back at a span of 4 seconds, Please click on the button to start the test and Wait for 5 seconds");
    },[]);
    ...
    return(
        <div className="LettersShow" >
            ...
            <div className="LettersStartScreen" style={{position:"relative",width:"100%",height:"60%",marginTop:"20%"}}>
                <p style={{color:"white",fontSize:"12px"}}>Note:You will be showned 26 letters back to back,</p>
                ...
                <div className = "LetterInstructions">
                    <button onClick = {() => speak({ text: "You will be showned 26 letters back to back, first one will be for practice, where score will not be counted. Please click on the button to start the test" })} className = "LetterInstructionsBtn" > <i className = "fas fa-volume-up" > </i></button>
                </div>
