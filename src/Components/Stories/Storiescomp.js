...
const StoriesComp=()=>{
    ...
    async function GetUserSentenceTestScore(){
        ...
        const SentenceApiResp = await fetch('https://iamadityachakraborty.pythonanywhere.com/GetWordsTestScore',{
            method:"POST",
    ...
    async function CompareStories(Value){
        ...
        const SentenceApiResp = await fetch('https://iamadityachakraborty.pythonanywhere.com/CompareStories',{
            method:"POST",
    ...
    if (IsMicrophonPopUp) {
        ...
        StoryShow={WordShow} Storyshowonmice={isSpeech} size={{fontSize:"12px"}} ShowingTrans={{visibility:"hidden"}} Isdisabled="disabled"> </SpeechRecognitionsComp>;
    };
    async function WordsPretestScoreSubmit(){
        ...
        const SendData = await fetch('https://iamadityachakraborty.pythonanywhere.com/SetStoryTestScore',{
            method:"POST",
    ...
    return(
        ...
        <div className="FinalPopUpResult" style={FinalResultPopupDisplay}>
            <FinalResultSentencePopUpComp ResultFinal={countRef.current} NextTest={NextTest} total={QuestionsList.length} ></FinalResultSentencePopUpComp>
        </div>
        ...
        <div className="SentenceStartscrrencontains" style={{position:"relative",width:"100%",height:"60%",marginTop:"20%"}}>
            ...
            <div className = "SentenceInstructions">
                    <button onClick = {() => speak({ text: "You will be showned a story, You have to read it on microphone, each minning words will be counted and few quistions will be asked from story, first one will be for practice where score will not be counted, Please click on the button to start the test" })} className = "SpeakerInstructions" > <i className = "fas fa-volume-up" > </i></button>
            </div>
