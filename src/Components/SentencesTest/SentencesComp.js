...
const SentencesShowComp = () => {
    ...
    if (IsMicrophonPopUp) {
        RecordingPopUps = <SpeechRecognitionsComp SubmitText = { PassAudioText } ShowingTrans={{visibility:"hidden"}} StoryShow={SentenceShow} size={{fontSize:"12px"}} Isdisabled="disabled"> </SpeechRecognitionsComp>;
    ...
    async function SentencePretestScoreSubmit(){
        ...
        //https://iamadityachakraborty.pythonanywhere.com/ <---Replace this before build
        const SendData = await fetch('https://iamadityachakraborty.pythonanywhere.com/SentenceTestScore',{
            method:"POST",
    ...
    useEffect(()=>{
        Say("You will be showned 5 Sentences back to back in a span of 10 seconds,Please click on the button to start the test");
    },[]);
    ...
    <div className = "SentencesShow" >
        ...
        <div className = "SentencesStartScreen" style = {isStartButton } >
            ...
            <p style = {{ color: "white", fontSize: "12px" }} > Note: You will be showned 5 Sentences back to back, </p> 
            ...
            <div className = "SentenceInstructions">
                    <button onClick = {() => speak({ text: "You will be showned 5 Sentences back to back, in a span of 10 seconds, Please click on the button to start the test" })} className = "SpeakerInstructions" > <i className = "fas fa-volume-up" > </i></button>
            </div>
